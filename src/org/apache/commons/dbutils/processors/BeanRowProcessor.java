...
public class BeanRowProcessor<T> implements RowProcessor<T> {
	...
	public BeanRowProcessor(Class<T> type){
		this.type = type;
		propertyDescriptors = propertyDescriptors(type);
	}
	
	@Override
	public T handleRow(ResultSet rs) throws SQLException {
		if(null == type){
			throw new SQLException("没有初始化类型信息!");
		}
		
		int[] columnToProperty = this.mapColumnsToProperties(rs.getMetaData());

		return this.createBean(rs, type, propertyDescriptors, columnToProperty);
	}
	
	private PropertyDescriptor[] propertyDescriptors(Class<T> type){
     ...
     */
    protected synchronized int[] mapColumnsToProperties(ResultSetMetaData rsmd
    		) throws SQLException {
    	if(null == this.columnToProperty){    		
     ...
     */
    private T createBean(ResultSet rs, Class<T> type,
     ...
     */
    protected T newInstance(Class<T> c) throws SQLException {
        try {
